...
import { LocalWatched } from "@/app/[locale]/vistas/LocalWatched";
import { BrandStar } from "@/components/BrandStar";
import { Card } from "@/components/ui/card";
import { Link } from "@/i18n/navigation";
import { createClient } from "@/lib/supabase/server";
import { getWatchedFromDb } from "@/lib/watched";

export const metadata = {
  title: "Ya las vi — MovieRate Compare",
};
...
export default async function VistasPage({ params }: Props) {
  ...
  if (!user) {
    return (
      <main className="px-4 sm:px-6 py-8 max-w-5xl mx-auto w-full">
        <h1 className="font-serif italic font-normal text-3xl sm:text-4xl leading-[0.95] tracking-tight text-balance mb-6">
          {t("heading")}
        </h1>
        <LocalWatched />
      </main>
  ...
  const items = await getWatchedFromDb(200);
    ...
    <main className="px-4 sm:px-6 py-8 max-w-5xl mx-auto w-full">
      ...
      {items.length === 0 ? (
        <Card className="p-8 text-center border-dashed">
          {/* Estrella casi vacía → "todavía nada visto" */}
